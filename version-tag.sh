...
if ! git diff-index --quiet HEAD -- || ! git diff --quiet --staged; then
    ...
    if [ "$choice" = "y" ]; then
        ...
        read -p "Введите сообщение коммита (или нажмите Enter для использования стандартного '$MESSAGE'): " custom_message
        ...
        # Пытаемся сделать коммит с выбранным сообщением
        if git commit -m "$commit_message"; then
            echo "Коммит '$commit_message' успешно создан."
        else
            echo "Ошибка: Не удалось создать коммит '$commit_message'."
